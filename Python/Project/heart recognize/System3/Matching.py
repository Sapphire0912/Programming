import MultiThreshold_v3 as Mt
...
class MatchModel(object):
    ...
    def MuscleMatching(self, LeftMValvePos, RightMValvePos, isOutputVideo=False):
        ...
        self._MedBlur(LeftMValvePos, RightMValvePos)  # 瓣膜中值濾波ㄎ
        ...
        while 1:
            ...
            # FrameContrast = Vp.ImageContrast(GrayFrame, 0, 150)
            # - 調整對比度 End.
            ...
            # try method 2: Matching LV Region  <- Matching 的過程要再處理一下, (模型在 Matching 的時候, 不能低於瓣膜位置)
            FitLVModel, BestLVResult, BestLVParam = ModelMatching(
                src=MT_target,
                target=A4CInfo["LVModel"]["src"],
                vertical=range(-20, 20, 5),
                horizontal=range(-30, 10, 5),
                theta=range(0, 40, 5)
            )
            ...
            # -- reg display
            # for part, PtsList in enumerate(TotalFitPts):
            #     for ret, Pts in enumerate(PtsList):
            #         if ret > 0:
            #             cv2.line(frame_target, tuple(Pts), tuple(PtsList[ret - 1]), colors[part - 1], 2)

            # 畫上 Apex Point
            # cv2.circle(frame_target, tuple(ApexPoint), 5, (0, 255, 0), -1)

            # frame_cp[y + minY:y + h, x:x + w] = frame_target
            # cv2.circle(frame_cp, LeftMVPts, 5, (0, 255, 0), -1)
            # cv2.circle(frame_cp, RightMVPts, 5, (255, 255, 0), -1)
            # cv2.line(frame_cp, LeftMVPts, RightMVPts, (255, 0, 0), 2)
            # cv2.putText(frame_cp, f'frame_count: {frame_count}', (40, 60), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,
            #             (255, 255, 255), 1)
            #
            # cv2.putText(frame_cp, 'Apical Septal', (40, 80), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.7,
            #             (0, 255, 0), 1)
            # cv2.putText(frame_cp, 'Septal', (40, 100), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.7,
            #             (255, 0, 0), 1)
            # cv2.putText(frame_cp, 'Basal Septal', (40, 120), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.7,
            #             (0, 0, 255), 1)
            # cv2.putText(frame_cp, 'Apical Lateral', (40, 140), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.7,
            #             (255, 255, 0), 1)
            # cv2.putText(frame_cp, 'Lateral', (40, 160), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.7,
            #             (0, 255, 255), 1)
            # cv2.putText(frame_cp, 'Basal Lateral', (40, 180), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.7,
            #             (255, 0, 255), 1)

            # cv2.imshow('frame', frame_cp)
            # cv2.imshow('BestLV', BestLVResult)
        ...
        # print(f'Original: {self.MuscleSemantic["Apical Septal"]}')
        ...
        if isOutputVideo:
            ...
            FileIO.write_video(matching_list, self.OutputMatchingDir + ' GLS_Pt2 School ' + FileName)
