...
class Segment(object):
    ...
    def write_video(self, frames_list, output_path):
    ...
    def Semantic_FindValve(self, isOutputSegVideo=False, isOutputValveVideo=True):
        ...
        while True:
            ...
            valve_list.append(DrawSemantic)

        if isOutputSegVideo:
            FileName = self.VideoPath.split('\\')[-1]
            self.write_video(semantic_list, self.OutputSegDir + ' semantic ' + FileName)

        if isOutputValveVideo:
            FileName = self.VideoPath.split('\\')[-1]
            self.write_video(valve_list, self.OutputSegDir + ' Valve ' + FileName)
