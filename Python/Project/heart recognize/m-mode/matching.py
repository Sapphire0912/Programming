...
def match_muscleA(frame, A, v_range=range(15, 80, 5), h_range=range(-40, 30, 5), t_range=range(-20, 20, 5)):
...
def match_muscleB(frame, B, v_range=range(-10, 10, 2), h_range=range(-80, -30, 5), t_range=range(-14, 14, 2)):
    ...
    for vertical in v_range:
        for horizontal in h_range:
            ...
            for theta in t_range:
                ...
                fitting_B = cv2.bitwise_and(rotate_B, frame)
...
def match_muscleC(frame, C, v_range=range(-32, 0, 4), h_range=range(-20, 6, 4), t_range=range(-6, 10, 2)):
    ...
    for vertical in v_range:
        for horizontal in h_range:
            ...
            for theta in t_range:
                ...
                fitting_C = cv2.bitwise_and(rotate_C, frame)
