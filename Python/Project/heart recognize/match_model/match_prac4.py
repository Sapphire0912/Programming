...
for path in all_video_path:
    ...
    while video.ret:
        ...
        # 報告過程展示
        # reg_ppt = np.zeros(frame.shape, np.uint8)
        # reg_ppt[mask_muscle == 255] = [255, 255, 255]
        # cv2.drawContours(reg_ppt, cnt_muscle_A, -1, (255, 0, 0), 2)
        # cv2.circle(reg_ppt, (center_muscle_ax, center_muscle_ay), 1, (255, 0, 0), 10)
        # cv2.drawContours(reg_ppt, cnt_muscle_B, -1, (0, 255, 0), 2)
        # cv2.circle(reg_ppt, (center_muscle_bx, center_muscle_by), 1, (0, 255, 0), 10)
        # cv2.drawContours(reg_ppt, cnt_muscle_C, -1, (0, 0, 255), 2)
        # cv2.circle(reg_ppt, (center_muscle_cx, center_muscle_cy), 1, (0, 0, 255), 10)
        # cv2.imshow('reg_ppt', reg_ppt)
        ...
        # 報告過程展示
        # reg_ppt = np.zeros(frame.shape, np.uint8)
        # reg_ppt[mask_filter == 255] = [255, 255, 255]
        # for i in range(len(lv_center)):
        #     ox, oy = lv_center[i]
        #     cv2.circle(reg_ppt, (ox, oy), 1, (255, 255, 0), 10)
        #
        # for i in range(len(la_center)):
        #     ox, oy = la_center[i]
        #     cv2.circle(reg_ppt, (ox, oy), 1, (0, 255, 255), 10)
        #
        # for i in range(len(aortic_center)):
        #     ox, oy = aortic_center[i]
        #     cv2.circle(reg_ppt, (ox, oy), 1, (255, 0, 255), 10)
        # cv2.circle(reg_ppt, (Bx, By), 1, (128, 255, 60), )
        # cv2.line(reg_ppt, (img_center_x, 0), (img_center_x, 600), (255, 0, 0), 2)
        # cv2.line(reg_ppt, (0, By), (800, By), (0, 255, 0), 2)
        # cv2.imshow('reg_ppt', reg_ppt)
        ...
        reg_bound_info = np.zeros(frame.shape[:2], np.uint8)
