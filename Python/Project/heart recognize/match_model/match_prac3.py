...
for path in all_video_path:
    ...
    mv_top = (677, 378)
    mv_bottom = (640, 477)
    cv2.line(mv_pos, mv_top, mv_bottom, (255, 255, 255), 15)

    # 動脈瓣膜位置(Aortic Value)
    av_pos = np.zeros(model_all.shape, np.uint8)
    av_top = (703, 339)
    av_bottom = (726, 391)
    cv2.line(av_pos, av_top, av_bottom, (255, 255, 255), 15)
    ...
    erode_bound_region = cv2.erode(gray_bound_region, cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)), iterations=15)
    # ----- Muscle bound End

    # ----- chamber marker
    # 利用 B 區段找到近似中心點(區分不同腔室的 marker)
    gray_B = cv2.cvtColor(model_best_B, cv2.COLOR_BGR2GRAY)
    cnt_B, _ = cv2.findContours(gray_B, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    x, y, w, h = cv2.boundingRect(cnt_B[0])
    # cv2.line(bound_all, (x, 0), (x, 600), (255, 255, 255), 5)

    # 避免 LA 和 AO 連在一起
    cv2.line(bound_all, (x+w, y+int(h/2)), (800, y+int(h/2)), (255, 255, 255), 20)
    bound_all[gray_bound_region != 255] = [0, 0, 0]

    # 將 LV, LA 分開(B 區段 左上角的點和 A 區段最接近的輪廓點連線)
    gray_A = cv2.cvtColor(model_best_A, cv2.COLOR_BGR2GRAY)
    cnt_A, _ = cv2.findContours(gray_A, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    distance = list()
    for i in range(len(cnt_A[0])):
        ax, ay = cnt_A[0][i][0]
        dis = np.sqrt((ax - x) ** 2 + (ay - y) ** 2)
        distance.append(dis)
    index = distance.index(min(distance))
    cv2.line(bound_all, (x+3, y), (cnt_A[0][index][0][0]+3, cnt_A[0][index][0][1]+6), (255, 255, 255), 10)
    gray_bound_all = cv2.cvtColor(bound_all, cv2.COLOR_BGR2GRAY)
    gray_muscle_marker = cv2.cvtColor(muscle_marker, cv2.COLOR_BGR2GRAY)
    # cv2.imshow('bound_all', bound_all)
    # ----- chamber marker End
    ...
    while video.ret:
        ...
        frame_cp = frame.copy()
        frame_cp[mask_roi != 255] = [0, 0, 0]
        frame_cp[erode_bound_region != 255] = [0, 0, 0]
        frame_cp[gray_bound_all == 255] = [0, 0, 0]
        frame_cp[gray_muscle_marker == 255] = [0, 0, 0]

        # 判斷注水要注到哪
        gray_frame = cv2.cvtColor(frame_cp, cv2.COLOR_BGR2GRAY)
        frame_inv = cv2.bitwise_not(gray_frame, mask=gray_frame)
        _, thres = cv2.threshold(frame_inv, 180, 255, cv2.THRESH_BINARY)
        closing = cv2.morphologyEx(thres, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)))
        # erode_frame = cv2.erode(closing, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)), iterations=1)

        filter_marker = np.zeros(frame_cp.shape, np.uint8)
        cnt_frame, _ = cv2.findContours(closing, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        for c in cnt_frame:
            if cv2.contourArea(c) > 500:
                cv2.drawContours(filter_marker, [c], -1, (255, 255, 255), -1)
        filter_marker = cv2.erode(filter_marker, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)), iterations=5)

        filter_marker = cv2.cvtColor(filter_marker, cv2.COLOR_BGR2GRAY)
        bg = cv2.dilate(erode_bound_region, cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)), iterations=2)
        unknown = cv2.subtract(bg, filter_marker)

        _, markers = cv2.connectedComponents(filter_marker)
        markers = markers + 1
        ...
        color_info[gray_muscle == 255] = [0, 0, 0]
        curr_result = cv2.addWeighted(frame, 1, color_info, 0.5, 1)
